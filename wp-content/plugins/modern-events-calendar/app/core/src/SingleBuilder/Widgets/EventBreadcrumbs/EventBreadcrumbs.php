	...
	public function output( $event_id = 0, $atts = array() ){
		...
		if ( true === $this->is_editor_mode && ( !isset($settings['breadcrumbs']) || !$settings['breadcrumbs'] ) ) {
			...
			$html = '<div class="mec-content-notification"><p>'
					.'<span>'. esc_html__('To show this widget, you need to enable "Breadcrumbs" module.', 'mec').'</span>'
					. '<a href="https://webnus.net/dox/modern-events-calendar/single-event-settings/#3_Breadcrumbs" target="_blank">' . esc_html__('Read More', 'mec') . ' </a>'
				.'</p></div>';
		} else {
