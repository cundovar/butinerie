	...
	public function output( $event_id = 0, $atts = array() ){
		...
		if ( true === $this->is_editor_mode && ( !isset($settings['social_network_status']) || !$settings['social_network_status'] ) ) {
			...
			$html = '<div class="mec-content-notification"><p>'
				.'<span>'. esc_html__('To show this widget, you need to enable "Social Networks" module.', 'mec').'</span>'
				. '<a href="https://webnus.net/dox/modern-events-calendar/event-modules/#Social_Networks" target="_blank">' . esc_html__('Read More', 'mec') . ' </a>'
			.'</p></div>';
		} else {
