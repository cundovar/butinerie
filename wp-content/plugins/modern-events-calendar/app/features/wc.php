...
class MEC_feature_wc extends MEC_base
{
    ...
    {
        if($meta->key == 'mec_event_id') $display_key = esc_html__('Event', 'mec');
        elseif($meta->key == 'mec_date') $display_key = esc_html__('Date', 'mec');
        elseif($meta->key == 'mec_other_dates') $display_key = esc_html__('Other Dates', 'mec');
        elseif($meta->key == 'mec_transaction_id') $display_key = esc_html__('Transaction ID', 'mec');
    ...
    public function display_value($display_value, $meta)
    {
        ...
        elseif($meta->key == 'mec_date')
        {
            ...
            $display_value = sprintf(esc_html__('%s to %s', 'mec'), $start_datetime, $end_datetime);
        }
        elseif($meta->key == 'mec_other_dates')
        {
            ...
            foreach($dates as $date)
            {
                ...
                $date_values[] = sprintf(esc_html__('%s to %s', 'mec'), $start_datetime, $end_datetime);
            }
    ...
    public function validate($data, $errors)
    {
        ...
        foreach($items as $item)
        {
            ...
            {
                $printed = true;
                if($availability[$ticket_id] == '0') $errors->add('validation', sprintf(esc_html__('%s ticket is sold out!', 'mec'), $tickets[$ticket_id]['name']));
                else $errors->add('validation', sprintf(esc_html__('Only %s slots remained for %s ticket so you cannot book %s ones.', 'mec'), $availability[$ticket_id], $tickets[$ticket_id]['name'], $quantity));
            }
        }
        ...
        foreach($all_items as $event_id => $tickets)
        {
            ...
            {
                $errors->add('validation', sprintf($this->main->m('booking_restriction_message3', esc_html__("Maximum allowed number of tickets that you can book is %s.", 'mec')), $limit));
            }
        }
    }
