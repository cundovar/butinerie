...
class MEC_feature_mec extends MEC_base
{
    ...
    public function revoke_license()
    {
        if ($this->getPRO())
        {
            ...
            $payload = json_encode(['message' => 'revoked', 'status' => true, 'button_text' => esc_html__('submit', 'mec')]);
            echo MEC_kses::element($payload);
            wp_die();
        }
    }
    ...
    public function import_settings()
    {
        ...
        if ($options == 'No-JSON')
        {
            echo '<div class="mec-message-import-error mec-error">' . esc_html__('Your option is not in JSON format. Please insert correct options in this field and try again.', 'mec') . '</div>';
            exit();
        }
        else
        {
            if (empty($options))
            {
                echo '<div class="mec-message-import-error mec-error">' . esc_html__('Your options field can not be empty!', 'mec') . '</div>';
                exit;
            }
            else
            {
                update_option('mec_options', $options);
                echo '<div class="mec-message-import-success mec-success">' . esc_html__('Your options imported successfuly.', 'mec') . '</div>';
            }
        }
    ...
    {
        add_submenu_page('mec-intro', esc_html__('MEC - Support', 'mec'), esc_html__('Support', 'mec'), 'manage_options', 'MEC-support', [$this, 'support_page']);
    }
    ...
    public function menus()
    {
        ...
        add_submenu_page('mec-intro', esc_html__('MEC - Settings', 'mec'), esc_html__('Settings', 'mec'), apply_filters('mec_menu_cap', $capability, 'settings'), 'MEC-settings', [$this, 'page']);

        add_submenu_page('mec-intro', esc_html__('MEC - Addons', 'mec'), esc_html__('Addons', 'mec'), apply_filters('mec_menu_cap', 'manage_options', 'addons'), 'MEC-addons', [$this, 'addons']);
        add_submenu_page('mec-intro', esc_html__('MEC - Wizard', 'mec'), esc_html__('Wizard', 'mec'), apply_filters('mec_menu_cap', 'manage_options', 'wizard'), 'MEC-wizard', [$this, 'setup_wizard']);

        if (!$this->getPRO()) add_submenu_page('mec-intro', esc_html__('MEC - Go Pro', 'mec'), esc_html__('Go Pro', 'mec'), apply_filters('mec_menu_cap', 'manage_options', 'go_pro'), 'MEC-go-pro', [$this, 'go_pro']);
    ...
    public function register_meta_boxes()
    {
        ...
        add_meta_box('mec_calendar_display_options', esc_html__('Display Options', 'mec'), [$this, 'meta_box_display_options'], 'mec_calendars', 'normal', 'high');
        add_meta_box('mec_calendar_filter', esc_html__('Filter Options', 'mec'), [$this, 'meta_box_filter'], 'mec_calendars', 'normal', 'high');
        add_meta_box('mec_calendar_shortcode', esc_html__('Shortcode', 'mec'), [$this, 'meta_box_shortcode'], 'mec_calendars', 'side');
        add_meta_box('mec_calendar_search_form', esc_html__('Search Form', 'mec'), [$this, 'meta_box_search_form'], 'mec_calendars', 'side');
    ...
    public function action_links($actions, $post)
    {
        ...
        {
            // Duplicate Button
            $actions['mec-duplicate'] = '<a href="' . esc_url($this->main->add_qs_vars(['mec-action' => 'duplicate-shortcode', 'id' => $post->ID])) . '">' . esc_html__('Duplicate', 'mec') . '</a>';
        }
                    ...
                    <div class="w-box-content">
                        ...
                        <form class="mec-sells-filter" method="GET" action="">
                            ...
                            <input type="text" class="mec_date_picker" name="start"
                                   placeholder="<?php esc_attr_e('Start Date', 'mec'); ?>"
                                   value="<?php echo esc_attr($start); ?>"/>
                            <input type="text" class="mec_date_picker" name="end"
                                   placeholder="<?php esc_attr_e('End Date', 'mec'); ?>"
                                   value="<?php echo esc_attr($end); ?>"/>
                            <select name="type">
                                <option
                                    value="daily" selected="selected"><?php esc_html_e('Daily', 'mec'); ?></option>
                                <option
                            ...
                            <select name="chart">
                                <option
                                    value="bar" selected="selected"><?php esc_html_e('Bar', 'mec'); ?></option>
                                <option
                                    value="line"><?php esc_html_e('Line', 'mec'); ?></option>
                            </select>
                ...
                <div class="w-box total-bookings print-events">
                    ...
                    <div class="w-box-content">
                        <form method="GET" action="<?php echo home_url(); ?>" target="_blank">
                            ...
                            <select name="mec-year" title="<?php esc_attr_e('Year', 'mec'); ?>">
                            ...
                            <select name="mec-month" title="<?php esc_attr_e('Month', 'mec'); ?>">
                                <?php for ($i = 1; $i <= 12; $i++): ?>
                                    <option
    ...
    public function register_assets_per_page_meta_boxes()
    {
        ...
        add_meta_box('mec_metabox_app', esc_html__('Include MEC Assets', 'mec'), [$this, 'meta_box_assets_per_page'], $post_types, 'side', 'low');
    }
        ...
        foreach ($event_fields as $field_id => $event_field)
        {
            ...
            <div class="mec-form-row">
                ...
                <select class="mec-col-12"
                    ...
                    <option
                        value="0" <?php if (isset($field_options[$field_id]) and isset($field_options[$field_id]['type']) and $field_options[$field_id]['type'] == '0') echo 'selected="selected"'; ?>><?php esc_html_e('Disabled', 'mec'); ?></option>
                    <?php if (in_array($type, ['text', 'textarea'])): ?>
                        <option
                            value="text_input" <?php if (isset($field_options[$field_id]) and isset($field_options[$field_id]['type']) and $field_options[$field_id]['type'] == 'text_input') echo 'selected="selected"'; ?>><?php esc_html_e('Text Input', 'mec'); ?></option>
                    <?php elseif ($type === 'date'): ?>
                        <option
                            value="date-range-picker" <?php if (isset($field_options[$field_id]) and isset($field_options[$field_id]['type']) and $field_options[$field_id]['type'] == 'date-range-picker') echo 'selected="selected"'; ?>><?php esc_html_e('Date Range Picker', 'mec'); ?></option>
                    <?php else: ?>
                        <option
                            value="dropdown" <?php if (isset($field_options[$field_id]) and isset($field_options[$field_id]['type']) and $field_options[$field_id]['type'] == 'dropdown') echo 'selected="selected"'; ?>><?php esc_html_e('Dropdown', 'mec'); ?></option>
                    <?php endif; ?>
                </select>
            </div>
            <?php
